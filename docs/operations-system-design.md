...
运营轨
  content/<操作>-<目标>   例: content/update-t1-p5
                            ...
                            content/update-broadcaster-laochen
```
...
注册 `get-preview-url` 工具，push 后自动从 git 获取最新 commit hash，拼接 Cloudflare Pages 预览链接 `https://<hash>.ffxiv-race-stats.pages.dev`。避免 Agent 手动推算 URL 时出错。
...
| 环境 | 触发方式 | URL | 用途 |
|------|---------|-----|------|
| **Preview** | push 到 `feature/*` / `content/*` 分支 | `<hash>.ffxiv-race-stats.pages.dev` | 开发自测 / 运营视觉确认 |
