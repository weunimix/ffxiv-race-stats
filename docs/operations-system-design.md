...
运营人员 ──飞书──→ PI Agent ──→ GitHub ──→ Cloudflare Pages ──→ CDN
                ...
                ├── Skills     ├── CI 校验（硬约束）
                ├── Rules      ├── CODEOWNERS
                └── pi-feishu / feishu-mcp   └── Branch Protection
```
...
运营人员与 Agent 的全部交互通过 **飞书** 完成，无需接触 Git 或终端：

| 通道 | 实现 | 用途 |
|------|------|------|
| 消息通道 | `pi-feishu` 扩展（飞书官方 Bot API，WebSocket 长连接） | 运营在飞书单聊或群聊中用自然语言 / @ 机器人指挥 Agent；Agent 回复、工具执行进度实时回传飞书 |
| 云文档 | `feishu-mcp`（MCP server，经 `pi-mcp-adapter` 桥接） | Agent 读写飞书云文档 / Wiki，操作细节见 `feishu-docs` skill |

**飞书凭证存储位置**：飞书 App ID / App Secret 存放在项目级 `.pi/settings.json`（pi-feishu 读取）与全局 `~/.pi/agent/mcp.json`（feishu-mcp 的 env），均 gitignore 不入库。`feishu-mcp` 以 tenant 身份访问应用自有云空间；读写他人文档 / 知识库需先在飞书网页端给应用授权。
...
| `add-broadcaster` | 转播方、直播间、添加转播、删除转播 | 增删改 broadcasters[] 中的条目 |

**工具型 Skill —— `feishu-docs`：** 上述三个 Skill 面向 `data.js` 数据操作。`feishu-docs` 是不同类别——它封装飞书云文档的 MCP 操作指南（文档读写、Wiki 的 node_token / obj_token 转换、block 树结构等），供运营要求查看 / 编辑 / 创建飞书文档时触发，不涉及 `data.js`。

数据层顶层域与 Skill 的覆盖关系：`teams[]` / `news[]` / `broadcasters[]` 由上述三个数据 Skill 覆盖；`meta` / `notices[]` / `sponsors[]` 暂由 Agent 按 Rules 直接处理（如这些域需要高频操作，可再为其增加专用 Skill）。

### 5.4 硬约束机制

合并与预览的约束通过 Skill 流程 + CI 共同保证：

**合并门禁** — Skill 内置流程 + 运营确认：

- Skill 规定"不收到运营回复'合并'，绝对不执行 merge"
- 合并前用 `gh pr view --json state,statusCheckRollup` 确认 CI 通过

**预览链接** — Skill 内按分支名净化构造：

```
https://<sanitized-branch>.ffxiv-race-stats.pages.dev
```

分支名净化规则：`/` → `-`，非字母数字字符 → `-`，全小写；或从 Cloudflare Pages bot 的 PR 评论获取。

**关键限制：** Cloudflare Pages 分支别名截断到 **28 字符**。`content-` 前缀占 8 字符，因此 `content/` 后面的部分不得超过 20 字符，否则别名被截断、预览 URL 无法推算。

**代码级硬约束 = CI 文件范围检查**（见 §6）：`content/*` 仅含 `data.js`、`feature/*` 不含 `data.js`，违规直接 FAIL。这是物理层面的越界阻断，独立于 Agent 自觉——Agent 即便误操作，也会被 CI 拦下。
...
│   │   ├── update-broadcaster.md#    触发 add-broadcaster skill
...
│   │   ├── add-broadcaster/
│   │   │   └── SKILL.md
│   │   └── feishu-docs/         #    飞书云文档 MCP 操作指南（工具型）
│   │       └── SKILL.md
│   ├── rules/                   # Rules — 质量约束
│   │   ├── content-style.md     #    中文风格、术语规范
│   │   └── data-integrity.md    #    数据完整性、边界值检查
│   ├── extensions/README.md     # 硬约束层说明（概念）
│   └── scripts/                 # 参考脚本（骨架，非可加载扩展）
│       ├── merge-gate.ts
│       └── preview-url.ts
│
├── schema/                      # 数据契约
...
| **阶段 4** | 硬约束（合并门禁 + 预览链接 + CODEOWNERS，经 CI + Skill 实现） | ✅ 完成 |
| **阶段 5** | Schema 驱动校验 + 文件拆分（constants.js / schema/ / data.js） | ✅ 完成 |
| **阶段 6** | 双向 CI 硬阻断（dev PR 禁改 data.js，content PR 禁改其他） | ✅ 完成 |
| **阶段 7** | 飞书接入（pi-feishu 消息通道 + feishu-mcp 云文档） | ✅ 完成 |

## 附录 C：当前部署现状（2026-06-28）

> 生产 Agent 部署在腾讯云 Ubuntu 服务器。本节记录实际部署状态（设计意图见正文，有出入处以本节为准）。

### Agent 运行环境
- 主机：腾讯云 CVM（Ubuntu 24.04.4 LTS，2 vCPU / 1.9G RAM）
- 常驻方式：vvterm tmux 会话（腾讯 ORCA Web 终端持久会话），交互式运行（非 systemd）
- 工作目录：`~/projects/FFXIVRanking`（main）—— pi 启动时加载项目 `.pi/` 角色 + 飞书凭证
- pi：`@earendil-works/pi-coding-agent` 0.77.0（Node v22.22.3，自包含 pi-node）

### 模型
- provider `minimax-cn`，经 `~/.pi/agent/settings.json` 覆盖为 OpenAI 兼容端点（`api.minimaxi.com/v1`）
- 默认模型 `MiniMax-M3`，thinking `medium`

### 第三方插件（pi 扩展）
- `pi-feishu` —— 飞书消息通道（运营交互入口，WebSocket 长连接）
- `pi-mcp-adapter` —— MCP 桥接

### MCP servers（`~/.pi/agent/mcp.json`）
- `feishu-docs` —— `npx -y feishu-mcp@latest --stdio`（飞书云文档读写，15 工具）

### Skills / Prompts / Rules（项目 `.pi/`，随 git）
- skills：`update-team`、`add-news`、`add-broadcaster`、`feishu-docs`
- prompts：`update-team`、`add-news`、`add-broadcaster`、`check-data`
- rules：`content-style`、`data-integrity`

### GitHub 访问
- gh CLI 2.45.0（鉴权 weunimix，repo 写权限）+ `gh auth setup-git`
- git/gh 经 mihomo 代理（per-host `github.com` → `127.0.0.1:7890`；飞书 / npm / DeepSeek 等国内服务直连）
