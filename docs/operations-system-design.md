...
飞书凭证（App ID / App Secret）存于项目级 `.pi/settings.json`，已加入 `.gitignore` 不入库。`feishu-mcp` 以 tenant 身份访问应用自有云空间；读写他人文档 / 知识库需先在飞书网页端给应用授权。
...
**工具型 Skill —— `feishu-docs`：** 上述五个 Skill 面向 `data.js` 数据操作。`feishu-docs` 是不同类别——它封装飞书云文档的 MCP 操作指南（文档读写、Wiki 的 node_token / obj_token 转换、block 树结构等），供运营要求查看 / 编辑 / 创建飞书文档时触发，不涉及 `data.js`。
